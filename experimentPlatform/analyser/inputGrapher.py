...
def phase_2_time_vs_field():
    inputStatisticaler.plot_box_solve_time_per_field()

    test_users = [key for key, val in userPRIV_to_pub.items() if val in ["andreiAll", "Duncan", "Alpaca"]]
    inputStatisticaler.plot_avg_solve_time_per_field_per_person(test_users, userPRIV_to_pub)
    inputStatisticaler.plot_avg_percent_difference_per_person(userPRIV_to_pub, seed_3bv_lookup, successful_only=True, time_unit="s")
